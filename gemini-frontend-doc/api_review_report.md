...
export const behaviorService = {
    ...
    batchLog: async (events: Array<{ type: string; timestamp: number; metadata?: any }>) => {
        return api.post<ApiResponse<{ received: number; processed: number }>>('/behavior/batch-log', { events });
    }
};
```
...
export const interactionService = {
    like: async (payload: LikePayload) => { /* existing */ },
    favorite: async (payload: FavoritePayload) => {
        const { data } = await api.post<ApiResponse<FavoriteResponse>>('/interactions/favorite', payload);
        return data.data;
    }
};
```
...
export const authService = {
    sendCode: async (payload: SendCodePayload) => { /* existing */ },
    login: async (payload: LoginPayload) => { /* existing */ },
