...
function normalizeQuestion(raw: any) {
    ...
    return {
        ...
        tags:     raw.tags     ?? [],
        stats:    { likes, favorites, comments, answers, views: raw.stats?.views ?? raw.views ?? undefined },
    };
}
...
export function useQuestionDetail(questionId: string) {
    ...
    useEffect(() => {
        ...
        questionService
            ...
            .then((q) => { if (!cancelled && q) setRawQuestion(q); })
            .catch(() => {})
    ...
    useEffect(() => {
        ...
        answerService
            ...
            .catch(() => {})
            .finally(() => { if (!cancelled) setIsLoadingAnswers(false); });
        return () => { cancelled = true; };
    }, [questionId]);
    ...
    useEffect(() => {
        ...
        commentService
            .listByQuestion(questionId)
            .then((res) => { if (!cancelled && res && Array.isArray(res.list)) setComments(res.list); })
            .catch(() => {})
    ...
    const handleLike = async () => {
        if (!currentUser) { toast.error('请先登录'); navigate('/login'); return; }
        if (!question) return;
        const nextLiked = !liked;
        setLiked(nextLiked);
        try {
            await interactionService.like({ targetType: 'question', targetId: question.id, action: nextLiked ? 'like' : 'unlike' });
            await behaviorService.log('question_like', { questionId: question.id, action: nextLiked ? 'like' : 'unlike' });
    ...
    const handleFavorite = async () => {
        ...
        try {
            await interactionService.favorite({ questionId: question.id, action: nextFavorited ? 'favorite' : 'unfavorite' } as any);
            await behaviorService.log('question_favorite', { questionId: question.id, action: nextFavorited ? 'favorite' : 'unfavorite' });
    ...
    const handlePlayAudio = () => {
        ...
        if (isPlayingAudio) {
            el.pause(); setIsPlayingAudio(false); toast.success('暂停播放');
        } else {
            el.play().then(() => { setIsPlayingAudio(true); toast.success('开始播放'); })
               .catch(() => { toast.error('无法播放音频，请稍后重试'); });
        }
    };

    const handlePlayAnswerAudio = (answerId: string) => {
        const currentAudio = answerAudioRefs.current[answerId];
        if (!currentAudio) { toast.error('音频加载中，请稍后重试'); return; }
        if (playingAnswerId === answerId) {
            currentAudio.pause(); setPlayingAnswerId(null); toast.success('暂停播放'); return;
        }
        if (playingAnswerId && answerAudioRefs.current[playingAnswerId]) {
            answerAudioRefs.current[playingAnswerId]?.pause();
        }
        currentAudio.play()
            .then(() => { setPlayingAnswerId(answerId); toast.success('开始播放'); })
            .catch(() => { toast.error('无法播放音频，请稍后重试'); });
    };
    ...
    const handleAddImage = () => {
        ...
        if (isTestEnv) {
            setCommentImage('https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?w=400&h=300&fit=crop');
            toast.success('已添加图片');
            return;
        }
    ...
    const handleSubmitComment = async () => {
        if (!currentUser) { toast.error('请先登录'); navigate('/login'); return; }
        if (!question) return;
        if (!newComment.trim() && !commentImage) { toast.error('请输入评论内容或上传图片'); return; }
        const canComment = currentUser.id === question.authorId || currentUser.role === 'teacher';
        if (!canComment) { toast.error('仅提问者和回答者可评论'); return; }

        if (USE_MOCK) {
            const comment: Comment = {
                id: `c${Date.now()}`, questionId: question.id, questionTitle: question.title,
                content: newComment, image: commentImage || undefined,
                authorId: currentUser.id, authorName: currentUser.name || currentUser.nickname || '',
                authorAvatar: currentUser.avatar, status: 'pending', aiResult: '无违规',
                createdAt: new Date().toISOString(),
            };
            setComments(prev => [...prev, comment]);
            setNewComment(''); setCommentImage(null);
            toast.success('评论已提交，等待审核');
            return;
        }

        try {
            const created = await commentService.create(question.id, { content: newComment.trim(), image: commentImage || undefined });
            const aiAudit = (created as any)?.aiAudit;
            if (aiAudit && !aiAudit.safe) { toast.error(`评论被拒绝：${aiAudit.reason || '内容不符合规范'}`); return; }
            if (created.status === 'approved') setComments(prev => [created, ...prev]);
            try { await behaviorService.log('question_comment', { questionId: question.id, hasImage: !!commentImage }); } catch {}
            setNewComment(''); setCommentImage(null);
    ...
    return {
        ...
        liked, favorited,
        isPlayingAudio, playbackRate, setPlaybackRate,
        playingAnswerId, selectedImage, setSelectedImage,
        showSpeedMenu, setShowSpeedMenu,
        questionAudioRef, answerAudioRefs, answerCardRefs,
        highlightAnswerId,
        // actions
        handleLike, handleFavorite,
        handlePlayAudio, handlePlayAnswerAudio,
        handleAddImage, handleCommentImageFileChange, handleSubmitComment,
        handleDelete,
        setTargetAnswerId,
    };
}
