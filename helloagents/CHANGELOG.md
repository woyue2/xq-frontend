...
### Changed
- **Profile 页面换绑手机号（前后端最小闭环）**:
  - 前端 `src/pages/ProfilePage.tsx` 新增“换绑手机号”入口与弹窗（新手机号 + 验证码两个纵向输入区），并提供换绑提交能力；
  - 换绑弹窗内“获取验证码”采用与注册页一致的展示节奏：成功后延迟 2 秒再缓慢显示“本次验证码”，文案显示在验证码按钮下方；
  - 后端新增 `POST /api/users/me/change-phone`（登录态）并在 `UserService.changePhone` 中完成验证码校验、手机号冲突校验与用户手机号更新；
  - `auth.sendCode` 扩展 `change_phone` 类型，换绑链路复用现有验证码存储机制。
- **注册验证码提示显示节奏优化（前端）**:
