...
1. **错误码风格差异**
   ...
   - 当前实现：
     - 使用 HTTP 状态码 + 字符串错误码：
       - 类型不支持 → `400 / INVALID_UPLOAD_TYPE`
       - 大小限制目前不在 Node 后端做检查（由 OSS 或前端控制）。
   - 影响：
     - 如果前端或测试用例按数字业务码（2001/2002）写死断言，会出现不一致；
     - 如果只看 HTTP 状态码 + `error` 字段，语义是合理的。
...
1. **响应结构差异**
   ...
   - 当前实现返回：
     ```json
     {
       "code": 200,
       "message": "Logged successfully",
       "data": { "id": "...", "success": true, "receivedAt": 1234567890 }
     }
     ```
   - 影响：字段名与 message 文案不同，但语义等价；前端如果只关心「是否成功」，当前版本是足够的。
...
3. **业务错误码规范 vs 实际实现**
   ...
   - 当前整体后端实现采用的是：
     - `code`: HTTP 状态码；
     - `error`: 业务字符串（如 `INVALID_PHONE_FORMAT`、`MEMBER_EXPIRED`、`RATE_LIMITED`）；
   - 这不仅限于行为埋点，是全局风格差异。要调整的话应该整体评审，而不是只改某一个接口。
