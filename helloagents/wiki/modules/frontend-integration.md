...
### 八、登录/注册验证码提示显示优化（2026-02-15）

- 文件：`src/pages/LoginPage.tsx`
- 变更点：
  - 注册态发送验证码成功后，“本次验证码：xxxxxx”提示改为延迟 2 秒显示；
  - 提示采用缓慢淡入动画，避免瞬时跳变；
  - 增加定时器清理机制，在页面状态切换与组件卸载时重置提示状态，防止残留展示。
- 兼容性说明：
  - 登录态行为保持不变；
  - ⚠️ 当后端未返回 `code` 字段时，不展示“本次验证码”提示文案（以避免展示空值）。

### 九、Profile 换绑手机号（2026-02-15）
...
- 文件：
  ...
  - `src/types/api.ts`
- 变更点：
  - 个人中心“账号操作”区新增“换绑手机号”入口（所有角色可见）；
  - 换绑弹窗包含“新手机号”“验证码”两个纵向输入区；
  - 弹窗内“获取验证码”按钮下方新增验证码提示文案，采用“延迟 2 秒 + 缓慢显示”的动效策略；
